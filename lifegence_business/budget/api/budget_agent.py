...
def get_budget_variance(
	...
	try:
		...
		if not company:
			return {"success": False, "error": "会社データが見つかりません。"}
		...
		if not plans:
			return {"success": False, "error": f"{fiscal_year}の承認済み予算計画が見つかりません。"}
		...
		for plan in plans:
			...
			for item in items:
				result = frappe.db.sql("""
					...
					WHERE cost_center=%s AND account=%s AND fiscal_year=%s AND company=%s AND is_cancelled=0
				""", (plan.cost_center, item.account, fiscal_year, company))
				actual = flt(result[0][0]) if result and result[0][0] else 0
				total_actual += actual
...
def get_forecast_landing(
	...
	try:
		...
		if not forecasts:
			return {"success": False, "error": "着地見込みデータが見つかりません。"}
...
def create_budget_plan_draft(
	...
	try:
		...
		if not cost_center:
			return {"success": False, "error": f"部門 '{department}' に原価センターが設定されていません。"}
		...
		if not prev_fy:
			return {"success": False, "error": "前年度の会計年度が見つかりません。"}
		...
		if not actuals:
			return {"success": False, "error": f"前年度（{prev_fy}）の実績データが見つかりません。"}
